...
use App\Context\Shared\UI\Http\Auth\OAuth2UserCredentials;
...
class AuthInit implements ModuleInit
{
    ...
    private static function initOAuthServer(Container $container): void
    {
        ...
        $scopeUtil = new Scope(new Memory(array(
            'default_scope' => $config->get('context_auth::oauth.scope'),
            'supported_scopes' => $config->get('context_auth::oauth.allowed_scopes')
        )));

        $server = new Server($storage, [
            'access_lifetime' => $config->get('context_auth::oauth.token_expire'),
        ]);
        $server->addGrantType(new ClientCredentials($storage));
        $server->addGrantType(new UserCredentials($credentials));
        $server->addGrantType(new RefreshToken($storage, [
            'always_issue_new_refresh_token' => $config->get('context_auth::oauth.always_issue_new_refresh_token'),
        ]));
        $server->setScopeUtil($scopeUtil);

        $container->set('context_oauth_server', $server);
    }
}
